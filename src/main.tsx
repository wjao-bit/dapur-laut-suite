...
// ============================================================================
// Self-healing untuk "chunk basi" (stale chunk):
//
// Setelah aplikasi dirilis ulang, file aset berubah hash (mis.
// AppLayout-abc.js → AppLayout-xyz.js). Kalau browser masih memegang
// index.html LAMA (dari cache HTTP/SW), ia akan mencoba memuat chunk lama
// yang sudah tidak ada di server → error "Failed to fetch dynamically
// imported module" → layar putih di route yang di-lazy-load.
//
// Solusi: deteksi error tersebut lalu muat ulang halaman SEKALI. Setelah
// muat ulang, index.html terbaru diambil (service worker memakai
// no-store untuk navigasi) dan chunk yang benar ikut termuat — aplikasi
// pulih sendiri tanpa perlu pengguna menghapus cache manual.
// ============================================================================
function installStaleChunkReload() {
  const KEY = "dl-stale-reload-v1";
  ...
  const reloadOnce = () => {
    ...
    try {
      ...
      /* sessionStorage tidak tersedia (sandbox) — reload tetap aman */
    }
    if (already) return; // sudah reload sekali — hindari loop tak berujung
    console.warn("[StaleChunk] Muat ulang untuk memulihkan dari chunk basi…");
    window.location.reload();
  };

  window.addEventListener("error", (e) => {
    if (looksStale(String(e.message || ""))) reloadOnce();
  });
...
const MonitorPage = lazy(() => import("./pages/app/MonitorPage.tsx"));
...
// updateViaCache: "none" → file sw.js itu sendiri tidak pernah diambil dari
// HTTP cache browser, jadi rilis baru SW selalu langsung terpasang.
