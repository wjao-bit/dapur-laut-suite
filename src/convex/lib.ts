...
export async function recomputeKas(ctx: MutationCtx) {
  const entries = await ctx.db.query("kas" as any).collect();
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    if (b && (b.saldoAwal !== e.saldoAwal || b.saldoAkhir !== e.saldoAkhir)) {
      await (ctx.db as any).patch(e._id, { saldoAwal: b.saldoAwal, saldoAkhir: b.saldoAkhir });
    }
  }
}
...
export async function recordKas(
  ...
  if (existing) {
    await (ctx.db as any).patch(existing._id, { tanggal, kasMasuk, kasKeluar, keterangan, sumber: sumber ?? "" });
  } else {
    await (ctx.db as any).insert("kas", {
      id: refKey,
      tanggal,
      kasMasuk,
      kasKeluar,
...
export async function ensureGudangRow(ctx: MutationCtx, namaBarang: string) {
  ...
  if (!existing) {
    await (ctx.db as any).insert("gudang", {
...
export async function addStokHistory(
  ...
  await (ctx.db as any).insert("stokHistory", {
...
export async function purgeStokFor(ctx: MutationCtx, namaBarang: string) {
  ...
  const hist = await (ctx.db.query("stokHistory") as any).filter((q: any) => q.eq(q.field("namaBarang"), namaBarang)).collect();
  for (const h of hist) await ctx.db.delete(h._id);
  const tStok = await findOneByKey(ctx, "tetesanStok" as any, "namaBarang", namaBarang);
  if (tStok) await ctx.db.delete(tStok._id);
  const tHist = await (ctx.db.query("tetesanStokHistory") as any).filter((q: any) => q.eq(q.field("namaBarang"), namaBarang)).collect();
  for (const h of tHist) await ctx.db.delete(h._id);
}

/**
 * Catat aktivitas admin (monitoring/audit).
 */
export async function logAktivitas(
  ...
  await (ctx.db as any).insert("aktivitas", {
