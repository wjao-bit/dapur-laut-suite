...
import { AlertCircle, ArrowRight, KeyRound, Loader2, LockKeyhole, Phone, Ship, UserPlus } from "lucide-react";
...
function Auth({ redirectAfterAuth }: AuthProps = {}) {
  ...
  const resetPassword = useMutation(api.admin.resetPasswordPublic);
  ...
  /** Lupa password: reset langsung dengan nomor HP + password baru (min. 6 karakter). */
  const handleReset = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const fd = new FormData(event.currentTarget);
      const phone = normalizePhone(String(fd.get("phone") || ""));
      const pwBaru = String(fd.get("newPassword") || "");
      ...
      await resetPassword({ phone, newPassword: pwBaru });
      setNotice("Password berhasil direset — silakan masuk dengan password baru.");
      setMode("login");
    } catch (err: any) {
        ...
        <Card className="w-full max-w-sm border-white/10 bg-white/95 shadow-2xl backdrop-blur">
          <CardHeader className="text-center">
            ...
            <CardTitle className="text-xl tracking-tight">
              {mode === "login" ? "Masuk Admin" : mode === "register" ? "Daftar Akun Admin" : "Ganti Password"}
            </CardTitle>
            <CardDescription>
              {mode === "login"
                ...
                : mode === "register"
                  ...
                  : "Lupa password? Isi nomor HP & password baru untuk mengatur ulang."}
            </CardDescription>
          </CardHeader>
          ...
          {mode === "login" ? (
            <form onSubmit={handleLogin}>
              ...
              <CardFooter className="flex-col gap-1.5">
                ...
                <Button
                  type="button"
                  variant="link"
                  className="w-full cursor-pointer text-xs text-sky-700"
                  onClick={() => {
                    setMode("reset");
                    setError(null);
                  }}
            ...
            <form onSubmit={handleReset}>
              <CardContent className="space-y-3">
                <div>
                  <Label htmlFor="reset-phone" className="text-xs font-medium">
                    Nomor HP terdaftar
                  </Label>
                  <div className="relative mt-1.5">
                    <Phone className="absolute top-1/2 left-3 size-4 -translate-y-1/2 text-muted-foreground" />
                ...
                <div>
                  <Label htmlFor="reset-new" className="text-xs font-medium">
                ...
                <p className="text-[11px] leading-4 text-muted-foreground">
                  Berlaku untuk akun yang sudah disetujui admin. Semua reset dicatat di log aktivitas
                  agar Admin Master bisa memantau.
                </p>
              </CardContent>
              <CardFooter className="flex-col gap-2">
                <Button type="submit" className="w-full cursor-pointer" disabled={isLoading}>
