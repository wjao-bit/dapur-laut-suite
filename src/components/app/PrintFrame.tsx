 ...
 *
 * `waText` (opsional): bila diisi, toolbar menampilkan tombol "Kirim WhatsApp"
 * yang membuka wa.me dengan teks ringkasan dokumen (mis. isi invoice).
 */
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-2">
          {waText && (
            <Button
              size="sm"
              variant="outline"
              className="h-9 whitespace-nowrap border-emerald-300 bg-emerald-50 px-3 text-emerald-700 hover:bg-emerald-100 sm:h-auto sm:px-4"
              onClick={() => {
                const url = `https://wa.me/?text=${encodeURIComponent(waText)}`;
                window.open(url, "_blank", "noopener,noreferrer");
              }}
              title="Kirim ringkasan dokumen ini ke WhatsApp"
            >
