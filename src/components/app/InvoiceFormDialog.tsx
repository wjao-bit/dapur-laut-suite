...
export function InvoiceFormDialog({
  ...
  useEffect(() => {
    ...
    setTipe(editInv?.tipe ?? "Reseller");
    setTanggal(editInv?.tanggal ?? todayStr());
    setIdInvoice(editInv?.idInvoice ?? nextInvoiceId);
    setNamaPihak(editInv?.namaPihak ?? "");
    setTenggat(editInv?.tenggat ?? "");
    setMataUang(editInv ? (editInv.mataUang === "$" ? "$" : "Rp") : "Rp");
  ...
  useEffect(() => {
    if (open && !editInv && !idTouched.current) setIdInvoice(nextInvoiceId);
  }, [open, editInv, nextInvoiceId]);
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [open, namaPihak, katalogForParty]);
